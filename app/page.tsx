...
function parseSheet(csv: string): { months: MonthData[]; reviewLabel: string } {
  ...
  return { months: months.filter(m => m.total > 0), reviewLabel };
}
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, padding: "24px 28px", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 8, flexWrap: "wrap" }}>
        ...
        <KPI label="Total Projected" value={fmt(totalProjected.total)} sub={projVariance > 0 ? `${fmt(projVariance)} under plan` : `${fmt(Math.abs(projVariance))} over plan`} good={projVariance >= 0} color={projVariance < 0 ? C.orange : undefined} />
